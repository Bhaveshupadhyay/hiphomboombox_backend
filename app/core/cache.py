        ...
        async def wrapper(*args, **kwargs):
            ...
            if result is not None:
                try:
                    ...
                    await redis_client.set(name=cache_key, value=data_to_store, ex=redis_ttl)
                except Exception as e:
                    logger.warning(f"[CACHE ERROR] Failed to store in Redis for {cache_key}: {e}")
            return result
        return wrapper
    return decorator
...
async def insert_redis_data(
    ...
    redis_client = get_redis_client()
    if redis_client:
        await redis_client.set(name=get_redis_key(namespace=namespace, key_parts=[key]), value=json.dumps(data), ex=redis_ttl)
