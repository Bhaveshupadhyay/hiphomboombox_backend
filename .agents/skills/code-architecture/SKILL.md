...
└── core/            # 6. Infrastructure (Config, DB Engine, Redis, DI)
```
...
1. **`app/api/v1/` (Route Handlers)**
   ...
   - **NO business logic or raw SQL queries allowed in route handlers.**
...
1. **Strict 5-Layer Parity**:
   Every new domain entity (e.g., `artist`, `album`) MUST have a corresponding file in each layer:
